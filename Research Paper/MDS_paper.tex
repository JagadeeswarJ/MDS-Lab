...
\begin{abstract}
When it is an emergency, traffic conditions in cities can delay ambulances by significant lengths of time. To solve this problem, we created an AI-based computer vision system using YOLOv8~\cite{yolov8ultralytics} to autonomously detect ambulances and other vehicles, through existing traffic cameras, in order to change traffic signals adaptively to create a "green wave" for emergency vehicles through traffic signals. The custom-trained model achieved a mean Average Precision (mAP@0.5) of 0.780 when detecting ambulances, which is a 105\% improvement in vehicle detection over baseline models. The model is able to classify five vehicle types (Ambulance, Bus, Car, Motorcycle, Truck) in real-time. We also validated the method using a Streamlit web application which demonstrated the potential for real world implementation of intelligent traffic management~\cite{traffic_management}.
\\[6pt]
...
The emergence of deep learning has ushered in a new era in vehicle detection; Convolutional Neural Networks (CNNs) demonstrated that it is possible to learn high-level hierarchies of feature representation from the raw image data without manual feature extraction~\cite{trafficvision2023}. The implementation of this framework permitted consistent and robust vehicle detection across varying conditions through training on large, publicly available datasets with a variety of conditions.
...
Single-stage detectors were developed as a solution to this speed limitation. Single Shot Detector (SSD) and the YOLO (You Only Look Once) family~\cite{redmon2016yolo} predict object locations and classifications in a single forward pass through the network. YOLOv8~\cite{yolov8ultralytics} is the latest version produced by Ultralytics that incorporates multiple architectural advancements such as improved backbone networks, feature pyramid structures, and better detection heads that are anchor-free. These improvements allow YOLOv8 to achieve state-of-the-art detection accuracy for all object sizes while retaining fast enough inference speeds to be used for real-time video analysis~\cite{yolov8comparison}.

\subsection{Emergency Vehicle Detection Research}

While research has been conducted on general vehicle detection, less attention has been given to specialized systems focused on emergency vehicle detection. Some work has explored audio-based siren detection~\cite{emergency2022}, but these systems suffer from urban noise pollution---honking horns, construction equipment, and street sounds create challenging acoustic environments. Audio approaches also require extensive microphone deployment and raise privacy concerns.

Visual detection via computer vision represents a more promising alternative. However, most vehicle detection datasets and models treat all vehicles generically without prioritizing emergency vehicles~\cite{trafficvision2023}. Chen et al.~\cite{emergency2022} developed a CNN-based approach for emergency vehicle detection using two-stage detection (Faster R-CNN), achieving good accuracy but limiting real-time performance. Patel et al.~\cite{ambulance2024} explored intelligent traffic signal control using computer vision, focusing on system integration. Wang et al.~\cite{yolov8comparison} compared YOLO variants for traffic scenarios, finding YOLOv8 offers optimal balance of accuracy and speed.

Our work builds upon these foundations by developing a system specifically optimized for ambulance detection with YOLOv8n, explicitly addressing deployment considerations---model size, inference speed, and confidence threshold tuning---essential for real-world traffic management adoption~\cite{traffic_management}.
...
We enlarged our dataset to utilize a rich Kaggle dataset~\cite{kaggle_vehicle} created for urban vehicle detection use cases. It characterizes exactly what we need: all of the images in this dataset were captured in a variety of real-world conditions---camera angles, times of day (morning, afternoon, evening), weather (sunny, cloudy, rainy), and traffic density. All of this variety is essential to create a robust model that can generalize to unseen traffic situations.
...
We selected YOLOv8~\cite{yolov8ultralytics} for its optimal balance of speed and accuracy in real-time video analysis. Building upon the original YOLO framework~\cite{redmon2016yolo}, YOLOv8 processes entire images in a single pass through three components: backbone (feature extraction using CSPDarknet architecture), neck (multi-scale feature aggregation via PANet), and head (anchor-free predictions with decoupled classification and localization branches).

We used YOLOv8n (nano variant) containing 3.2 million parameters, optimized for fast inference (30-50+ FPS) on standard hardware without costly GPU requirements. This lightweight design enables edge deployment at traffic intersections while maintaining sufficient accuracy for emergency vehicle detection~\cite{yolov8comparison}.

Training utilized the Ultralytics framework with the following configuration: 10 epochs on 640×640 pixel images, SGD optimizer (learning rate 0.01, momentum 0.937), cosine annealing schedule, and data augmentation including mosaic augmentation, random flipping, and HSV color space variations. Cloud infrastructure with GPU acceleration minimized training time. We monitored performance using mean Average Precision (mAP@0.5), which assesses both classification accuracy and localization precision. The highest-performing checkpoint was saved as \texttt{best.pt} for deployment.

\subsection{System Implementation and Validation}

To illustrate real-world relevance, we built an interactive web application using Streamlit~\cite{streamlit2023}, a Python framework which allows for hassle-free deployment of machine learning applications and user-friendly dashboards. The application is intended to be both a proof of concept and a demonstration tool for all parties: traffic officials, city planners, or emergency services coordinators.

\paragraph{Technical Architecture}
The backend of the application utilizes multiple important Python libraries. First, Ultralytics~\cite{yolov8ultralytics} provides loading and inference of the YOLOv8 model. OpenCV~\cite{opencv2023} performs processing on video streams, frame extraction, rendering of bounding boxes, and overlaying the text within the frames. Pillow handles image format conversions. NumPy provides useful numerical operations on the image arrays. Finally, Streamlit provides a responsive web interface with minimal coding effort.
...
The confidence threshold determines the minimum prediction confidence required to accept a detection as valid. Figure~\ref{fig:f1_confidence} presents the F1-Confidence curves for all vehicle classes, showing how detection performance varies with different threshold settings.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{img4.png}
    \caption{F1-Confidence Curves for All Vehicle Classes. The thick blue line represents overall F1-score, peaking at 0.51 at confidence threshold 0.323. The ambulance class (blue) maintains superior and stable performance across a wide confidence range (0.1-0.7), consistently above 0.70 F1-score.}
    \label{fig:f1_confidence}
\end{figure}

The overall F1-score peaks at 0.51 with a confidence threshold of 0.323, representing optimal balance between precision and recall across all classes. Notably, the ambulance class demonstrates the strongest and most stable performance, maintaining F1-scores above 0.70 across confidence values from 0.1 to 0.7. This robustness provides operational flexibility for traffic authorities to adjust thresholds based on priorities: lower thresholds (0.20-0.25) maximize ambulance detection recall for high-traffic areas, while higher thresholds (0.45-0.55) reduce false alarms in sensitive intersections. The bus class shows second-best performance (F1: 0.65-0.67), while car, motorcycle, and truck classes exhibit more modest scores (0.35-0.45) due to higher intra-class variability and occlusion challenges~\cite{trafficvision2023}.

\subsection{Qualitative Results and Detection Examples}

Figure~\ref{fig:detection_examples} presents a gallery of detection results across diverse real-world traffic scenarios, demonstrating the model's practical performance.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.98\textwidth]{img5.jpg}
    \caption{Gallery of Detection Results Across Diverse Traffic Scenarios. The figure shows multiple test cases demonstrating ambulance and vehicle detection under various conditions including different viewing angles, lighting conditions, vehicle types, and occlusion levels. Bounding boxes are color-coded by class with confidence scores displayed.}
    \label{fig:detection_examples}
\end{figure}

The gallery reveals several key performance characteristics. Ambulances are detected with high confidence scores (>0.9) across different designs---from compact medical vans to larger emergency response vehicles and fire trucks---demonstrating generalization capability despite limited training data. The model successfully handles various viewpoints (frontal, side, three-quarter angles), lighting conditions (daylight, overcast, shadows), and scale variations (close-up to distant vehicles).

Multi-class detection performs effectively, with motorcycles identified at confidence scores around 0.8-0.9 despite their small size, and proper discrimination between visually similar classes (ambulances vs. buses/trucks). The model demonstrates robustness to partial occlusion and dense traffic scenes, though confidence scores predictably decrease for heavily occluded or very distant objects. The temporal consistency visible across consecutive frames (same ambulance detected with similar confidence) validates the system's suitability for real-time video analysis and supports implementation of temporal filtering strategies to further improve deployment reliability~\cite{emergency2022,ambulance2024}.
